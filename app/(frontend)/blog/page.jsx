...
function getCatColor(category = "") {
  ...
  for (let i = 0; i < category.length; i++)
    h = (h + category.charCodeAt(i)) % CATEGORY_COLORS.length;
...
function MetaRow({ blog, className = "" }) {
  ...
  return (
    <div
      className={`flex flex-wrap items-center gap-3 text-xs text-slate-500 ${className}`}
    >
      {author && (
    ...
    <article className="group relative flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
      ...
      <div className="relative aspect-[16/10] overflow-hidden bg-slate-100">
        {blog.coverImage ? (
          ...
          <div
            className={`absolute inset-0 flex items-center justify-center ${c.bg}`}
          >
            <svg
              width="36"
              height="36"
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        <div className="mt-auto flex items-center justify-between border-t border-slate-100 pt-4">
          <div className="flex items-center gap-2 text-xs text-slate-500">
            ...
            <div>
              <p className="font-semibold text-slate-700 leading-tight">
                {blog.authorName || blog.author || "Author"}
              </p>
          ...
          <span
            className={`flex h-8 w-8 items-center justify-center rounded-full ${c.bg} ${c.text} group-hover:bg-indigo-100 group-hover:text-indigo-700 transition-colors shrink-0`}
          >
...
export default async function BlogPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA]">
      {/* ── Main content ── */}
      <main className="mx-auto max-w-360 px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        {blogs.length === 0 ? (
          /* Empty state */
          <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-200 bg-white py-28 text-center">
            ...
            <p className="mt-2 text-slate-400 max-w-sm">
              Check back soon — new articles appear here as soon as they're
              published.
            </p>
          </div>
        ) : (
            ...
            {rest.length > 0 && (
              <section>
                ...
                <div className="flex flex-col gap-4 md:hidden">
                  {rest.map((blog) => (
                    <WideCard key={blog.id} blog={blog} />
                  ))}
                </div>
